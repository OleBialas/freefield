        ...
        for i in speakers:
            ...
            for n in range(n_reps):
                ...
                while fs.get_variable(variable="recording", proc="RP2"):
                    time.sleep(0.01)
                time.sleep(0.5)
                rec = fs.get_variable(variable="data",n_samples=sound_in.nsamples+delay_samples, proc="RP2")
...
def record_hp_tf(sound="chirp",dur=0.04, n_reps=10, samplerate=48828.125):
    ...
    fs.trigger()
    left = fs.get_variable(variable="data_out_left",n_samples=sound_in.nsamples+delay_samples, proc="RP2")
    right = fs.get_variable(variable="data_out_right",n_samples=sound_in.nsamples+delay_samples, proc="RP2")
